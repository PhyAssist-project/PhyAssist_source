...
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer, LlamaForCausalLM, LlamaTokenizer, BitsAndBytesConfig, GenerationConfig
...
def load_complete_test_data(test_data_path):
    test_data = {}
...
def generate_response(model, tokenizer, test_data, device, batch_size):
    ...
    for dataset_name in test_data.keys():
        ...
        for i in range(0, len(queries), batch_size):
            ...
            batch_data = ["[INST]" + s + "[/INST]" for s in batch_data]
            print(batch_data)
            ...
            generate_ids = model.generate(input_ids, generation_config=GenerationConfig(max_new_tokens=256, pad_token_id = tokenizer.eos_token_id, \
                                                                                                eos_token_id = tokenizer.eos_token_id, attention_mask = attention_mask, \
                                                                                                 do_sample=True, temperature=0.4, top_p=0.6, repetition_penalty=1.3, num_return_sequences=1,))
            final_result = ""
...
def save_output(generated_text, goldens, output_path):
    if not os.path.exists("MentalLLaMA_dataset/model_output/"):
        os.mkdir("MentalLLaMA_dataset/model_output/")
    if not os.path.exists("MentalLLaMA_dataset/model_output/"+output_path):
        os.mkdir("MentalLLaMA_dataset/model_output/"+output_path)
    for dataset_name in generated_text.keys():
        ...
        output.to_csv("{}/{}/{}.csv".format('MentalLLaMA_dataset/model_output',
...
def main(model_path: str, adapter_path: str, model_output_path: str, batch_size: int, test_dataset: str, rule_calculate: bool, llama: bool, device: str, cuda: bool, test_data_path: str):
    if llama:
    ...
    if test_dataset == 'IMHI':
        ...
        test_data = load_complete_test_data(test_data_path)
