...
from BARTScore.BARTScore_src.bart_score import BARTScorer
...
def BARTscore(gen_dir_name, device):
    bart_scorer = BARTScorer(device=device, checkpoint='/data/IDE-project/MentalLLaMA_dataset/src/BARTScore/bart-large-cnn')
    bart_scorer.load(path='/data/IDE-project/MentalLLaMA_dataset/src/BARTScore/bart_score.pth')
    score_results = {}

    for root, ds, fs in os.walk("MentalLLaMA_dataset/model_output/{}".format(gen_dir_name)):
        for fn in fs:
